...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        timerHandler = Handler() // Initialize timerHandler here
    ...
    private fun startTimer(timerTextView: TextView, timerProgressBar: ProgressBar) {
        isTimerRunning = true
        val totalSeconds = 60
        timerSecondsRemaining = totalSeconds.toLong()

        timerProgressBar.max = totalSeconds
        ...
        timerRunnable = object : Runnable {
            override fun run() {
                if (timerSecondsRemaining <= 0) {
                    ...
                    timerTextView.text = formatTime(timerSecondsRemaining)
                    timerProgressBar.progress = (totalSeconds - timerSecondsRemaining).toInt()
    ...
    private fun pauseTimer(timerButton: Button) {
        isTimerRunning = false
